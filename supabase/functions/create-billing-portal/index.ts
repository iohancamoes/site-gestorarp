...
serve(async (req) => {
  ...
  try {
    ...
    // 4. Buscar Organization ID do usuário
    ...
    if (subError || !subData?.stripe_customer_id) {
      return new Response(JSON.stringify({ error: 'Stripe Customer ID not found for organization' }), { 
        status: 404, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      })
    }
    ...
    // 6. Criar sessão do portal de faturamento
